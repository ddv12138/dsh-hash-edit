...
import {
  xxh32, buildHashes, mapStableHashes, canon, idxToHash, idxOfHash, hashOfLine
} from '../src/engine.js'
...
test('same canonical content -> same base hash (before collision probing)', () => {
  assert.strictEqual(hashOfLine('}'), hashOfLine('}'))
...
test('anchors are stable across an insert edit', () => {
  ...
  const nl = ['a', 'bA', 'b', 'c', 'd', 'e']
  const h1 = mapStableHashes(h0, old, 1, 1, nl)
  assert.strictEqual(h1[0], h0[0])
  assert.strictEqual(h1[3], h0[2])
  assert.strictEqual(h1[4], h0[3])
  assert.strictEqual(h1[5], h0[4])
...
test('anchors are stable across a delete edit', () => {
  const old = ['a', 'b', 'c', 'd']
  const h0 = buildHashes(old, null)
  const dl = mapStableHashes(h0, old, 1, 1, ['a', 'c', 'd'])
...
test('identical file content reproduces identical anchors', () => {
  ...
  assert.deepStrictEqual(buildHashes(lines, null), buildHashes(lines, null))
})

// Reference value guard so a regression in xxh32 cannot silently pass.
test('xxh32 guard vector', () => {
  assert.strictEqual(xxh32('hashline anchor', 0), 0xa005e547)
})
